...
T1 dot(std::vector<T1>& x, std::vector<T2>& y)
{
    return std::transform_reduce(std::execution::par_unseq,x.begin(),x.end(),y.begin(),(T1) 0.0,std::plus<T1>{},
                [=](T1 xi, T2 yi){return xi + yi; });
}
